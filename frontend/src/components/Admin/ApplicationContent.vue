...
import { Chart, BarController, BarElement, ChartData, ChartOptions, registerables } from "chart.js";
Chart.register(BarController, BarElement, ...registerables);

const settingsStore = useSettingsStore();

// TODO: refactor dans un backofficeStore
...
export default defineComponent({
   ...
   async setup() {
      ...
      const roles = ref(await fetchRoles());
      const requests = ref(await fetchRequests());
      ...
      // TODO : mettre users en cache/store commun pour éviter les requêtes inutiles
      ...
      const filterSearch = (payload) => {
         ...
         const filtered = backupUsers.filter(u =>
            ...
            || u.roles.includes(e => e.includes(payload))
         );
      ...
      <div class="users h-full w-[49%] p-5 bg-white rounded shadow">
         <div class="header w-full h-[24px] mb-4 flex items-center justify-between opacity__animation">
            ...
            <div class="buttons w-[270px] h-full flex items-center justify-between">
               ...
               <button @click="showPopUp('userModify')" class="modify bg-white2 px-2 ml-2 w-[80px] h-full rounded-sm text-sm transition-[all] shadow" :disabled="!modifyActive" :class="modifyActive ? 'active' : ''">
                  Modifier
               </button>
            </div>
         </div>

         <div class="container w-full h-[91%] rounded-sm bg-white2 border-[1px] border-light-grey overflow-y-auto opacity__animation">
            <div class="header h-[40px] w-full bg-white flex flex-col">
            ...
            <div
               ...
               class="user relative w-full h-[32px] bg-white flex outline-none cursor-pointer transition-[all] z-10"
            >
      ...
      <div class="settings h-full w-[49%] flex flex-col justify-between">
         ...
         <div class="stats w-full h-[calc(100%-110px)] flex flex-col justify-between">
            <div class="request__stats w-full h-[49%] bg-white rounded shadow p-4">
               <BarChart v-bind="userBarChartProps" />
            </div>
            <div class="user__stats w-full h-[49%] bg-white rounded shadow p-4">
               <BarChart v-bind="requestBarChartProps" />
            </div>
         </div>
      </div>
   </div>
