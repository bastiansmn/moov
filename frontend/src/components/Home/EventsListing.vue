   ...
   async setup() {
      ...
      const filterEvents = ($event, tag) => {
         ...
         if (button.classList.contains("active")) {
            ...
            settingsStore.filter = settingsStore.filter.filter(item => item !== tag);
         } else {
            button.classList.add("active");
            settingsStore.filter.push(tag);
         }
      };

      let fetching = false;
      const onProgress = async (swiper, progress) => {
         if (progress > .6 && !fetching && settingsStore.filter.length === 0) {
            fetching = true;
...
<template>
   <div class="category__filter overflow-x-hidden opacity__animation">
      ...
      >
         ...
         >
            <button :class="`filterer min-w-fit h-1/2 flex items-center mx-3 px-4 bg-white rounded-[999px] ${settingsStore.filter.includes(tag) ? 'active' : ''}`"  @click="filterEvents($event, tag)">
